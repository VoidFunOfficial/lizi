...
使用 Node（`.nvmrc`）和项目锁定的 pnpm，执行 `pnpm install --frozen-lockfile`。提交会自动执行 Web、Android 和 iOS 模拟器构建；合并版本 PR 后自动创建 Release。Android 正式签名需要配置发布文档列出的 GitHub Secrets。
...
Mac 安装 Xcode 26+ 后，在项目根目录运行：

```bash
pnpm ios        # 编译模拟器 App，无需签名
pnpm ios:open   # 构建网页并打开 Xcode，供真机签名和安装
```

脚本会安装锁定依赖、运行测试与 lint、导出静态网页、创建或同步 iOS 工程、补齐定位权限说明并生成校徽图标。模拟器产物为 `outputs/ios/simulator/Build/Products/Debug-iphonesimulator/App.app`。

安装到 iPhone：连接手机并信任 Mac，在 Xcode 的 **Settings → Accounts** 登录 Apple 账号，然后在 **App target → Signing & Capabilities** 勾选 **Automatically manage signing** 并选择自己的 **Team**。如 Bundle Identifier 被占用，改成自己的唯一值。选择顶部的 iPhone，开启手机的开发者模式后按 **⌘R** 编译安装。

详细步骤、免费账号有效期、真机命令行编译和当前功能限制见 [iOS 编译与安装指南](docs/IOS.md)。
...
旧的 27 栋建筑预设已清空。现在在地图标注的「添加阴影」工具中，依次点击矩形两个对角，填写建筑高度（米），点击「保存阴影」。用「选择」点击矩形，可修改名称和高度、重画矩形或删除；所有修改支持撤销。建筑轮廓保存在地图文档的 `solarBuildings` 中，与路网一起自动保存、导出、导入；旧文件没有该字段时按空列表处理。标注轮廓在阴天也可见，`/debug_sunshine` 会同步读取同一份已保存建筑，提供全天阴影预览。

太阳高度角和方位角按江阴校区中心、所选绝对时间计算；阴影长度使用 `建筑高度 / tan(太阳高度角)`，方向与太阳方位相反。地图会显示同一组二维投影多边形，路线引擎也直接求道路折线落入这些多边形的实际长度。
