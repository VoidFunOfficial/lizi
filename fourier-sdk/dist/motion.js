...
var RootContext = createContext3(false);
function FourierMotion({ children }) {
  const nested = useContext3(RootContext);
  ...
  return React4.createElement(RootContext.Provider, { value: true }, children);
}
...
function transformFor(target) {
  const shortcuts = [...transformKeys].filter((key) => target[key] !== undefined);
  ...
  if (target.x !== undefined || target.y !== undefined || target.z !== undefined) {
    parts.push(`translate3d(${dimension(target.x)}, ${dimension(target.y)}, ${dimension(target.z)})`);
  }
  ...
  if (target.scaleY !== undefined)
    parts.push(`scaleY(${target.scaleY})`);
  if (target.rotate !== undefined)
    parts.push(`rotate(${angle(target.rotate)})`);
  if (target.rotateX !== undefined)
    parts.push(`rotateX(${angle(target.rotateX)})`);
  if (target.rotateY !== undefined)
    parts.push(`rotateY(${angle(target.rotateY)})`);
  if (target.skewX !== undefined)
    parts.push(`skewX(${angle(target.skewX)})`);
  if (target.skewY !== undefined)
    parts.push(`skewY(${angle(target.skewY)})`);
  return parts.join(" ");
}
function nativeKeyframe(target) {
  const frame = {};
...
function createMotionComponent(tag) {
  ...
  const Component2 = forwardRef2(function FourierMotionElement(rawProps, forwardedRef) {
    ...
    const insideRoot = useContext3(RootContext);
    const timeline = useFourierTimeline();
    const target = useRef4(null);
    const declaration = useRef4({
    ...
    useLayoutEffect3(() => {
      ...
      timeline.animate(target.current, keyframesFor(declaration.current.initial, declaration.current.animate), animationOptions(declaration.current.transition));
    }, [insideRoot, timeline]);
    const {
      initial: _initial,
      animate: _animate,
    ...
    return React4.createElement(tag, { ...elementProps, ref: setTarget });
  });
...
//# debugId=0202ED326204FE9764756E2164756E21
