...
Vercel 部署 `/` 官网、`/app` 导航、`/editor` 标注工具和静态资源。根 `api/weather.ts` 与 `api/student/jw.ts` 将共享的应用 API 作为 Vercel Node.js Functions 导出；天气代理、教务请求来源校验及大小/超时限制继续生效。
...
`outputs/vercel/njustmap` 是独立部署包，包含静态页和已打包的 Node API；GitHub CI 和 Release 中的 Web 附件也使用它。它可以通过 Vercel Drop 或 Vercel CLI 部署。普通 `pnpm build` 仍保留原来的 Worker 构建供现有开发工具使用，线上部署应使用 `pnpm build:vercel`。
